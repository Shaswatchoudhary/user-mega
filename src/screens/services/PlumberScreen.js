...
const PlumberScreen = ({ navigation, route }) => {
  ...
  // Get category from params, default to 'Plumber'
        ...
        const mappedWorkers = json.data
          .filter(w => {
            ...
            return normalizedWorkerCat === targetCat || 
                   (targetCat === 'plumber' && normalizedWorkerCat === 'plumbers') ||
                   (targetCat === 'plumbers' && normalizedWorkerCat === 'plumber');
          })
          .map(worker => ({
            id: worker._id,
  ...
  const filteredAndSortedWorkers = useMemo(() => {
    ...
    switch (selectedFilter) {
      ...
      case 'price_low': result.sort((a, b) => a.rate - b.rate); break;
      case 'price_high': result.sort((a, b) => b.rate - a.rate); break;
      ...
      <View style={styles.cardContent}>
        <View style={styles.avatarContainer}>
          <View style={styles.avatarPlaceholder}>
            <MaterialCommunityIcons name="water-pump" size={32} color="#9CA3AF" />
          </View>
        </View>
    ...
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerCenter}>
          ...
          <Text style={styles.headerSubtitle}>{filteredAndSortedWorkers.length} professionals nearby</Text>
        </View>
      ...
      >
        {filteredAndSortedWorkers.map(worker => renderWorkerCard(worker))}
      </ScrollView>
