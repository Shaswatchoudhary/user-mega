...
export default function NotificationScreen({ navigation }) {
  ...
  // Business ID is the identifier for notifications in the workers collection
  const workerId = workerProfile?.id || workerProfile?._id;
  ...
  const renderItem = ({ item }) => {
    const getIcon = () => {
      switch (item.type) {
        ...
        case 'job_request': return 'briefcase-plus';
    ...
    const getIconColor = () => {
      switch (item.type) {
        ...
        case 'job_request': return '#E84545';
    ...
    <SafeAreaView style={styles.container}>
      ...
      {notifications.length === 0 ? (
        <View style={styles.emptyContainer}>
          ...
          <Text style={styles.emptySubtext}>We'll notify you when you get new job requests or updates.</Text>
        </View>
      ) : (
        <FlatList
          data={notifications}
