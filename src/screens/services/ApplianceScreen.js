...
const ApplianceScreen = ({ navigation, route }) => {
  ...
  const category = route?.params?.category || 'Appliance Repair';
  ...
  const fetchWorkers = async () => {
    try {
      ...
      if (json.success && json.data) {
        console.log(`DEBUG: ApplianceScreen - Total workers from API: ${json.data.length}`);
        ...
        const mappedWorkers = json.data
          .filter(w => {
            ...
            const match = normalizedWorkerCat === targetCat || 
                         (targetCat === 'appliance' && normalizedWorkerCat === 'appliances') ||
                         (targetCat === 'appliances' && normalizedWorkerCat === 'appliance');
            return match;
          })
          .map(worker => ({
            id: worker._id,
      ...
      <View style={styles.cardContent}>
        <View style={styles.avatarContainer}>
          <View style={styles.avatarPlaceholder}>
            <MaterialCommunityIcons name="washing-machine" size={32} color="#9CA3AF" />
          </View>
        </View>
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      {/* Filter Modal - Simplified for brevity but functional */}
      <Modal
