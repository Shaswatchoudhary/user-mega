...
import { getAppCheck, initializeAppCheck, ReactNativeFirebaseAppCheckProvider } from '@react-native-firebase/app-check';
...
/**
 * 3. Firebase App Check (Modular)
 */
try {
  const appCheck = getAppCheck();
  const provider = new ReactNativeFirebaseAppCheckProvider(appCheck);
  provider.configure({
    android: {
      provider: 'debug',
