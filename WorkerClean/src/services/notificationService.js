...
class NotificationService {
  ...
  async saveFCMTokenToFirestore(token) {
    ...
    try {
      // In Worker app, token should be saved to workers collection
      await firestore()
        .doc(`workers/${this.userId}`)
        .set({
          fcmToken: token,
          updatedAt: firestore.FieldValue.serverTimestamp(),
        }, { merge: true });
      console.log('[NotificationService] Token saved for worker:', this.userId);
    } catch (error) {
  ...
  async onForegroundMessage(remoteMessage) {
    ...
    let notificationColor = '#E84545'; // default WorkEase brand red
    const type = data?.type || 'general';
    if (type === 'booking') {
      notificationColor = '#F59E0B'; // orange/amber for new booking requests to stand out
    } else if (type === 'job_closed') {
      notificationColor = '#22C55E'; // success green for completed/closed jobs
  ...
  async saveNotificationHistory(remoteMessage) {
    const { notification, data } = remoteMessage;
    try {
      // History should also be in workers collection for worker app
      await firestore()
        .doc(`workers/${this.userId}`)
