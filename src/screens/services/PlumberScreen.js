  ...
  const fetchWorkers = async () => {
    try {
      ...
      if (json.success && json.data) {
        console.log(`DEBUG: PlumberScreen - Total workers from API: ${json.data.length}`);
        console.log(`DEBUG: All Workers in DB:`, json.data.map(w => w.fullName || w.name));
        ...
        const mappedWorkers = json.data
          .filter(w => {
            ...
            if (w.fullName?.includes('Ritvik') || w._id?.includes('Ritvik')) {
              console.log(`DEBUG: Checking Ritvik - Cat: ${w.category}, Target: ${targetCat}, Match: ${match}, Status: ${w.status}`);
            }
          ...
          .map(worker => ({
            ...
            rate: worker.basePrice || worker.rate || 249,
            distance: 0.5,
            verified: true,
