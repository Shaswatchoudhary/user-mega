...
const ApplianceScreen = ({ navigation, route }) => {
  ...
  // Get category from params, default to 'Appliance Repair'
        ...
        const mappedWorkers = json.data
          .filter(w => {
            ...
            return normalizedWorkerCat === targetCat || 
                   (targetCat === 'appliancerepair' && normalizedWorkerCat === 'appliances') ||
                   (targetCat === 'appliances' && normalizedWorkerCat === 'appliancerepair');
          })
          .map(worker => ({
            id: worker._id,
