...
const OtpScreen = ({ navigation }) => {
  ...
  // API Base URL
  const API_URL = config.WORKER_API_BASE_URL;
    ...
    if (canResend) {
      ...
      try {
        ...
        if (result.success) {
          ...
          setIsLoading(false);
          const errorCode = result.code ? ` (${result.code})` : '';
          Alert.alert('Error', (result.error || 'Failed to resend OTP') + errorCode);
        }
      } catch (error) {
        setIsLoading(false);
  ...
  const handleVerifyOTP = async (enteredOtp = null) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        try {
          const syncResponse = await axios.post(`${config.API_BASE_URL}/auth/firebase-login`, {
            phone: phoneNumber,
            uid: result.user.uid
          });
          ...
          console.error('[WorkerAuth] Backend Sync Error:', syncError);
          setIsLoading(false);
          ...
          <View style={styles.ctaContainer}>
            ...
            >
              <Text style={styles.ctaText}>
                {isLoading ? 'Please Wait...' : (step === 1 ? 'Get OTP' : 'Verify OTP')}
              </Text>
            </TouchableOpacity>
